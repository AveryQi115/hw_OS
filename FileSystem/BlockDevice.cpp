 ...
 */
int BlockDevice::NSECTOR = 18000;	/* Max Possible Sector Numbers */
 ...
 */
BlockDevice g_ATADevice(&g_Atab);
...
int BlockDevice::Strategy(Buf* bp)
{
	...
	if(bp->b_blkno >= BlockDevice::NSECTOR)
	{
		/* 设置出错标志 */
...
void BlockDevice::Start()
{
	...
	/* 设置磁盘寄存器，启动I/O操作 */
	//TODO：这里真的开始IO，原ATADriver会写芯片，这里要改了
